...
export default function ConnectPanel({ session, onClose, onConnected, onAbort }) {
  ...
  const refreshQr = React.useCallback(async () => {
    try {
      ...
      // O runtime pode ainda estar inicializando; o polling continua.
    }
    ...
    pollRef.current = setInterval(async () => {
      try {
        ...
        if (typeof data?.qrcode === 'string') {
          ...
          setQrPhase('qr');
          startQrTimer();
        } else if (qrAttempts < 30) {
          qrAttempts += 1;
          await refreshQr();
        }
      } catch {
        // Mantém o polling para tolerar o boot do runtime.
      }
    }, 2000);
  }, [applyStatus, refreshQr, session.id, startQrTimer]);

  /* ── Runtime manager polling + init ── */
  React.useEffect(() => {
  ...
  }, [pollStatus, refreshQr, session.id, session.status]);
